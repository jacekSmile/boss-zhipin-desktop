...
- **应用内扫码登录**：安装包内置 Chrome for Testing 浏览器，离屏（隐藏）运行；登录二维码直接显示在应用内，用 BOSS直聘 App 扫码即可，无需打开任何浏览器窗口
...
- **批量投递**：勾选岗位加入队列，自定义打招呼话术模板（支持 `{公司}` `{职位}` `{自我介绍}` 变量），随机间隔（可配置）逐个发起沟通，**实时进度条 + 逐项日志**，可随时取消，失败/剩余项一键继续投递，自动记录投递历史与当日投递统计
- **消息沟通**：应用内查看会话列表、未读状态、聊天记录，直接回复 HR
- **简历管理**：上传/预览（PDF/文本）/删除简历，配合自我介绍要点个性化打招呼话术
- **拟人化反检测**：真实 Chrome 内核（非 headless）、无自动化特征标记、随机化请求间隔、风控码（31/37）与验证页识别，触发风控立即中止并引导人工处理（可随时唤出/隐藏浏览器窗口）

## 工作原理

应用启动时会拉起安装包内自带的 **Chrome for Testing**（离屏隐藏窗口运行，不是 headless，浏览器指纹与正常用户一致），通过 Chrome DevTools Protocol (CDP) 控制它访问 zhipin.com——与参考项目的 CDP 方案等价，但浏览器随安装包分发，开箱即用。登录二维码、职位搜索、JD 详情、打招呼、聊天全部通过它在已登录页面上下文中完成，登录态持久保存在应用数据目录。

> Linux ARM64 无官方 Chrome for Testing 构建，该平台的安装包不内置浏览器，运行时会回退使用系统已安装的 Chrome/Chromium。
